...
class Hospital:
    def __init__(self, root):
        ...
        DataframeLeft.columnconfigure(1, weight=1); DataframeLeft.columnconfigure(3, weight=1)
        ...
        self.combo_search['values'] = ("Ref", "Name"); self.combo_search.pack(side=LEFT, padx=5); self.combo_search.set("Ref")

        Entry(Searchframe, textvariable=self.search_txt, width=25, font=("arial", 11)).pack(side=LEFT, padx=5)
        Button(Searchframe, text="Search", bg="#f39c12", fg="white", width=10, font=("arial", 9, "bold"), command=self.search_data).pack(side=LEFT, padx=5)
        Button(Searchframe, text="Show All", bg="#3498db", fg="white", width=10, font=("arial", 9, "bold"), command=self.fetch_data).pack(side=LEFT, padx=5)
    ...
    def generate_prescription(self):
        ...
        p_text = f"Patient: {self.PatientName.get()}\nDOB: {self.DateOfBirth.get()}\nNHS: {self.nhsNumber.get()}\n"
        p_text += f"Medicine: {self.Nameoftablets.get()}\nRef: {self.ref.get()}\nDose: {self.Dose.get()}\n"
        p_text += f"Qty: {self.NumberofTablets.get()}\nIssue: {self.Issuedate.get()}\nExp: {self.ExpDate.get()}\n"
        p_text += "------------------------------------------\nTake as directed by physician."
        self.txtPrescription.insert(END, p_text)
    ...
    def print_bill(self):
        ...
        if data.strip() == "":
            messagebox.showwarning("Empty", "Pehle Bill ya Prescription generate karein")
            return
    ...
    def save_data(self):
        ...
        try:
            ...
            conn.commit(); conn.close(); self.fetch_data(); messagebox.showinfo("Success", "Record Saved")
    ...
    def fetch_data(self):
        try:
            ...
            if len(rows)!=0:
                ...
                for row in rows: self.hospital_table.insert("", END, values=row)
            conn.close()
        except: pass
    ...
    def delete_data(self):
        if self.ref.get()=="": messagebox.showerror("Error", "Select Record First")
        else:
            m = messagebox.askyesno("Confirm", "Delete karna chahte ho?")
            if m:
                conn = mysql.connector.connect(host="localhost", user="root", password="12345", database="hospital_db")
